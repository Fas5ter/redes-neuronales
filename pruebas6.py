...
def main():
    # Menú de selección de dataset
    opc = questionary.select(
        "¿Cuál dataset desea utilizar?",
        choices=["1. Iris.csv", "2. Bill_authentication.csv"]
    ).ask()
    ...
    elif opc == "2. Bill_authentication.csv":
    ...
    neuronasO = int(questionary.text("Ingrese el número de neuronas de la capa oculta:").ask())
    tasa_aprendizaje = float(questionary.text("Ingrese la tasa de Aprendizaje:").ask())
    epocas = int(questionary.text("Ingrese el número de épocas:").ask())

    # Selección de funciones de activación
    f_capa_oculta = questionary.select(
        "¿Qué función de activación desea utilizar en la capa oculta? (Default = Gaussiana)",
        choices=["1. Gaussiana", "2. Multicuadrática", "3. Multicuadrática inversa"]
    ).ask()[0]  # Tomamos el primer carácter como opción

    f_capa_salida = questionary.select(
        "¿Qué función de activación desea utilizar en la capa de salida? (Default = Softmax)",
        choices=["1. Sigmoide", "2. Softmax", "3. Tangente hiperbólica"]
    ).ask()[0]  # Tomamos el primer carácter como opción
    ...
    # Asignar funciones de activación seleccionadas
